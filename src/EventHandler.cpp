...
PF_Err EventHandler::HandleEvent(
    PF_InData* in_data,
    PF_OutData* out_data,
    PF_ParamDef* params[],
    PF_LayerDef* output,
...
PF_Err EventHandler::HandleMouseDown(
    PF_InData* in_data,
    PF_OutData* out_data,
    SequenceData* seqData,
    const PF_Point* point)
{
    PF_Point curve_point = ConvertToGraphCoords(point);
    
    // Add point to active curve if within bounds
    if (curve_point.h >= 0 && curve_point.h <= 1.0f &&
        curve_point.v >= 0 && curve_point.v <= 1.0f) {
        
        CurvePoint newPoint;
        newPoint.x = curve_point.h;
        newPoint.y = curve_point.v;
...
PF_Err EventHandler::HandleDrag(
    PF_InData* in_data,
    PF_OutData* out_data,
    SequenceData* seqData,
    const PF_Point* point)
{
    if (seqData->selected_point >= 0 && 
        ...
        PF_FpLong x = PF_MAX(0.0f, PF_MIN(1.0f, curve_point.h));
        PF_FpLong y = PF_MAX(0.0f, PF_MIN(1.0f, curve_point.v));
...
PF_Err EventHandler::HandleKeyDown(
    PF_InData* in_data,
    PF_OutData* out_data,
    SequenceData* seqData,
...
PF_Point EventHandler::ConvertToGraphCoords(const PF_Point* screen_point)
{
    PF_Point curve_point;
    curve_point.h = (screen_point->h - 10) / 256.0f;
    curve_point.v = 1.0f - ((screen_point->v - 10) / 256.0f);
    return curve_point;
}